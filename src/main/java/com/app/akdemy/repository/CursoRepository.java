...
public interface CursoRepository extends CrudRepository<Curso, Long> {
    ...
    @Query(value = "(SELECT  curso.id, cur_anio, cur_nombre, cur_director FROM curso JOIN horario_curso ON curso.id = hor_cur_curso WHERE cur_anio = 2022 AND hor_cur_profesor = :profesorID) UNION (SELECT * FROM curso WHERE cur_anio = year(current_date) AND cur_director = :profesorID)", nativeQuery = true)
